...
int main()
{
	...
	String xml_file_name = "TempletInformation-600px.xml";	// XML文件名
	vector< vector<KeyPoint> > key_points_arr;				// 所有模板图片的特征点vector
	vector<Mat> descriptor_arr;								// 所有图片的特征向量vector
	double t = (double)getTickCount();
	LoadXML(xml_file_name, key_points_arr, descriptor_arr);	// 加载XML文件
	...
	while (true)
	{
		while (true)
		{
			...
			imshow("Camera", frame);	// 在Camera窗口展示图像帧
			int k = waitKey(1);			// 等待按下按键
			if ((char)k == ' ')			// 如果按下的按键时空格键
			{
				img = frame;			// 将当前帧数据赋值给img
				destroyWindow("Camera");// 关闭Camera窗口
				break;
			}
		...
		double t_begin = (double)getTickCount();	// 用于统计整个匹配过程的总用时

		Ptr<SURF> detector = SURF::create(800);		// 创建SURF特征检测器
		vector<KeyPoint> key_points;				// 存储待匹配图片的特征点
		...
		{
			double t = (double)getTickCount();		// 用于计算其中一个模板的匹配用时
			...
			t = ((double)getTickCount() - t) / getTickFrequency();	// 算出这次匹配的用时（单位s）
